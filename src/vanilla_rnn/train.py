...
Y = dtype(all_types[TEST_SUBJECT_ID][0,:])
...
# for rnn, X should have shape (seq_len=1000, num samples, 22)
#swapped_X = Variable(np.swapaxes(unpadded_X, 1, 2).T) this'll happen interanlly

# for generality, we'll pack these into variable length arrays
X = ppseq(input = unpadded_X, lengths = T)
